...
export interface TrainerConfig {
  ...
  quiet: boolean;
  /** Vary the first move per game from a set of legal opening seeds so that
   *  self-play actually diverges (the engine is deterministic at a fixed
   *  start position, so plain self-play produces an identical game every time
   *  and the book never grows). */
  seedOpenings: boolean;
}

// Legal white opening moves from the standard 9x9 start position, expressed as
// [from {r,c}, to {r,c}]. White pawns sit on row 7, knights/archbishop/
// chancellor on row 8.
const OPENING_SEEDS: { from: { r: number; c: number }; to: { r: number; c: number } }[] = [
  { from: { r: 7, c: 4 }, to: { r: 5, c: 4 } }, // e3 (e-pawn double push)
  { from: { r: 7, c: 3 }, to: { r: 5, c: 3 } }, // d3 (d-pawn double push)
  { from: { r: 7, c: 2 }, to: { r: 5, c: 2 } }, // c3 (c-pawn double push)
  { from: { r: 7, c: 6 }, to: { r: 5, c: 6 } }, // g3 (g-pawn double push)
  { from: { r: 7, c: 1 }, to: { r: 5, c: 2 } }, // Nc3 / Ac3 / Cc3 (b1 knight-ish)
  { from: { r: 7, c: 6 }, to: { r: 5, c: 5 } }, // Nf3 (g1 knight-ish)
  { from: { r: 7, c: 1 }, to: { r: 5, c: 0 } }, // Na3 (b1 knight to a3)
  { from: { r: 7, c: 6 }, to: { r: 5, c: 7 } }, // Nh3 (g1 knight to h3)
  { from: { r: 7, c: 2 }, to: { r: 5, c: 4 } }, // Be3 (c1 bishop to e3)
];
...
export class OpeningBookTrainer {
  ...
  private async playTrainingGame(gameNum: number, swapColors: boolean): Promise<void> {
    ...
    // Optionally vary the opening: pick a random legal white seed move so that
    // self-play diverges (the engine is deterministic from a fixed position).
    if (this.config.seedOpenings) {
      const seed = OPENING_SEEDS[Math.floor(Math.random() * OPENING_SEEDS.length)];
      applyMoveInt(state.board, { from: seed.from, to: seed.to, capture: false });
      ...
      // Black's reply is generated by the engine below; we just recorded the
      // resulting position so the opening book tracks it.
  ...
  private recordGameResult(state: EngineState, result: 'win' | 'loss' | 'draw', swapColors: boolean): void {
    ...
    const moveHistory = state.moveHistory
      .slice(0, this.config.openingMovesTracked)
      .map(m => ({
        from: m.from,
        to: m.to,
        piece: 'p' as const,
  ...
  private finalizeBook(): BookData {
    ...
    return {
      ...
      metadata: {
        ...
        description: `Generated from ${this.stats.totalGames} engine self-play games${this.config.seedOpenings ? ' (seeded varied openings)' : ''}`,
...
async function main(): Promise<void> {
  ...
  for (let i = 0; i < args.length; i++) {
    switch (args[i]) {
      ...
      case '--openings': config.seedOpenings = true; break;
      case '--no-openings': config.seedOpenings = false; break;
