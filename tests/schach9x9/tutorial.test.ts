 ...
 * This proves close() removes exactly the listener that was added, and that the listener
 * count stays balanced across open/close cycles.
 */
describe('Tutorial keydown listener lifecycle', () => {
  ...
  it('attaches exactly one keydown listener on init', () => {
    new Tutorial();
    expect(added.filter(a => a.type === 'keydown')).toHaveLength(1);
  });
